...
def main():
    ...
    # resnet = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_resnet34', pretrained=True)
    ...
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=False, pin_memory=True, num_workers=2)
        ...
        for idx, loader in enumerate([train_loader, test_loader]):
            ...
            for images, l in tqdm(loader, desc=f'Iteration: {idx}'):
                ...
                if 'mnist' in args.dataset.lower():
                    output = resnet(images[:, :1].repeat(1, 3, 1, 1))
                else:
