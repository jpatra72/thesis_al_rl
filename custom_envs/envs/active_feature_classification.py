...
# TODO: use pin_memory?
class ImgFeatureClassifier(gym.Env):
    ...
    def __init__(self,
        ...
        full_shape = self.image_shape[1] + self.class_count + 1
    ...
    def _get_obs(self):
        ...
        _conf = self._confidence
        return np.concatenate((_image, _pred, _conf))
...
def test_env(model, dataloaders, al_budget, episode_count, model_kwargs):
    gym_env = gym.make('custom_envs/ActiveImgFeatureClassifier-v0',
                       model=model,
...
def test_env_vectorised(model, dataloaders, al_budget, episode_count, env_count, model_kwargs):
    # set context for python multiprocessing
    vec_kwargs = {
        'context': 'spawn'
    }
    envs = gym.make_vec('custom_envs/ActiveImgFeatureClassifier-v0',
                        num_envs=env_count,
...
if __name__ == '__main__':
    ...
    test_env(wrapped_CNN_model, dataloaders, al_budget=10, episode_count=10, model_kwargs=model_kwargs)
    finish_time_serial = time.time() - start_time

    start_time = time.time()
    test_env_vectorised(wrapped_CNN_model, dataloaders, al_budget=10, episode_count=10, env_count=4,
                        model_kwargs=model_kwargs)
