...
def plot_performance(models, dataset, df, fig):
    ...
    for idx in range(len(performance_mean)):
        plt.plot(data_percent, performance_mean[idx], color=c[c4_100_keys[idx]], label=labels[models[idx]],
                 marker='o', markersize=2.5, linewidth=1)
        plt.fill_between(data_percent, performance_min[idx], performance_max[idx],
...
def main():
    ...
    for idx, dataset in enumerate(['kmnist']):
        ...
        plot_performance(models, dataset, df, fig)
    ...
    #            borderaxespad=0.5)
    # fig.show()
    fig.subplots_adjust(bottom=0.22, top=0.92, left=0.10, right=0.90)
    # fig.show()
