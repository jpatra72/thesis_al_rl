...
def main():
    ...
    x, y = set_size(398,
                    subplots=(2, 2),  # specify subplot layout for nice scaling
                    fraction=1)
    # fig = plt.figure(figsize=(x, y)
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(x , y))
    fig.tight_layout()
    ax = ax.reshape(-1)

    models = ['resnet50', 'dino_single', 'dino_multi']
    for idx, dataset in enumerate(['mnist', 'kmnist', 'cifar', 'kmnist']):
    ...
    a = fig.legend(handles, labels,
               ...
               # bbox_to_anchor=(0, ),
               borderaxespad=0)
    # fig.subplots_adjust(bottom=0.25, top=0.91, left=0.08, right=0.97)
    fig.subplots_adjust(hspace=0.4)
    # plt.show()
    plt_folder_path = get_plots_subfolder_path('test_2x2')
