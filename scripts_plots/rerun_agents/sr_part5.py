...
def main(state, budget, skip, std):
    ...
    for idx, dataset in enumerate(['cifar']):
        ...
        plot_performance(df_dqn0, df_dqn1, df_baselines, baselines, ax, plot_order, dataset, budget, skip, std)
    ...
    plt_save_path = os.path.join(plt_folder_path, f'{state}_{budget}_{skip}_{int(std)}_cifar')
...
state = 'sr4'
a = 'cs' if state == 'sr3' else 'cp'
labels_dict = {'dqn0': 'SR1',
               'dqn1': f'SR1 without margin ($s^m$)',
               'margin': 'margin',
               'coreset': 'coreset',
               'rstream': 'rand-stream',
               'random': 'rand'}
